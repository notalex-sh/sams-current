<script>
  ...
  import { filteredEntries, stats, allTags, searchQuery, selectedTag, activeTab, sortBy } from '$lib/stores/database';
...
<div class="max-w-6xl mx-auto content-load h-full flex flex-col overflow-hidden">
  ...
  <div class="grid lg:grid-cols-3 gap-4 flex-1 min-h-0 overflow-hidden">
    ...
    <div class="lg:col-span-2 flex flex-col min-h-0 overflow-hidden">
      <div class="flex border-b border-white/10 mb-2 overflow-x-auto flex-shrink-0">
        {#each tabs as tab}
          <button
            ...
            on:click={() => { $activeTab = tab.id; if (entriesContainer) entriesContainer.scrollTop = 0; }}
          >
            {tab.label}
            {#if tab.id === 'expiring' && $stats.expiring > 0}
              <span class="ml-1 px-1 py-0.5 text-[10px] bg-white/10 border border-white/20">{$stats.expiring}</span>
            {/if}
          </button>
        {/each}
      </div>
      ...
      <div bind:this={entriesContainer} class="space-y-2 custom-scrollbar flex-1 overflow-y-auto pr-1 min-h-0">
        ...
        {:else}
          {#each $filteredEntries as entry (entry.id)}
            <EntryCard {entry} on:notify={(e) => dispatch('notify', e.detail)} />
          {/each}
        {/if}
      </div>
