...
<div class="w-full max-w-md px-4 fade-in">
  <div class="cyber-border p-8 shadow-2xl">
    ...
    {#if !isCreatingNew}
      <div class="mb-6">
        <label class="block text-xs uppercase tracking-wider text-white/40 mb-2">Database File</label>
        <div class="flex gap-2">
          <input type="text" readonly value={selectedFile ? selectedFile.name : 'NO FILE SELECTED'} class="mono-input flex-1 text-xs text-white/50" />
          <label class="mono-button-secondary cursor-pointer text-xs px-3">
            Open
            <input type="file" accept=".sams" class="hidden" on:change={handleFileSelect} />
          </label>
        </div>
        <div class="mt-4 text-center">
          <span class="text-xs text-white/20 uppercase tracking-wider">or</span>
        </div>
        <button class="mono-button w-full mt-4" on:click={startNewDatabase}>Create New Database</button>
      </div>
    {/if}
    ...
    {#if selectedFile || isCreatingNew}
      <div class="space-y-4 fade-in">
        <div>
          ...
          <input type="password" bind:value={password} placeholder="••••••••" class="mono-input" on:keydown={handleKeyDown} />
        </div>

        {#if isCreatingNew}
          <div>
            <label class="block text-xs uppercase tracking-wider text-white/40 mb-2">Confirm Password</label>
            <input type="password" bind:value={confirmPassword} placeholder="••••••••" class="mono-input" on:keydown={handleKeyDown} />
          </div>
        ...
        <div class="flex gap-2 pt-2">
          <button class="mono-button flex-1" on:click={handleSubmit} disabled={loading}>
            {#if loading}
          ...
          {#if isCreatingNew}
            <button class="mono-button-secondary" on:click={() => { isCreatingNew = false; password = ''; confirmPassword = ''; }}>Cancel</button>
          {/if}
        </div>
      </div>
    {/if}
